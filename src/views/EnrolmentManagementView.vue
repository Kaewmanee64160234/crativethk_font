...
const delEnrolment = (idEnrollment: number, nameCourse: string) => {
  messageStore.showConfirm_(
    `ต้องการออกจากวิชา ${nameCourse} ใช่หรือไม่`,
    () => confirmDelEnrolment(idEnrollment),
    () => {
        ...
        >
          <v-img height="100" :src="randomImage()" cover>
            ...
            <v-menu offset-y>
              ...
              <v-list>
                <v-list-item
                  @click="delEnrolment(item.enrollmentId!,item.course?.nameCourses!)"
                >
          ...
          <v-card-text>
            <div class="text-body" style="font-weight: bold">
              อาจารย์ {{ item.course?.user?.firstName }}
            </div>
            <div class="text-body">
              เวลาเริ่มเรียน Lecture
              {{
                item.course?.dayInLec +
                " " +
                item.course?.timeInLec +
                " น." +
                " - " +
                item.course?.timeOutLec +
                " น."
              }}
            </div>
            <div class="text-body" v-if="item.course?.typeCourses === 'Lecture & Lab'">
              ...
              {{
                item.course?.dayInLec +
                " " +
                item.course.timeInLab +
                " น." +
                " - " +
                item.course.timeOutLab +
                " น."
              }}
            </div>
            <div v-else>
