...
const images = ref<string[]>(userStore.editUser?.images?.map((image: string) => `${url}/users/image/filename/${image}`) ?? []);
...
onMounted(async () => {
  images.value = userStore.editUser?.images?.map((image: string) => `${url}/users/image/filename/${image}`) ?? [];
...
async function save() {
  if (canUpload.value) {
    userStore.editUser = {
      ...userStore.editUser,
      firstName: userStore.editUser!.firstName || '',
      lastName: userStore.editUser!.lastName || '',
